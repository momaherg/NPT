...
class NPTDecoderLayer(LlamaDecoderLayer):
    ...
    def __init__(self, config, layer_idx: int = None):
        ...
        self.np_component = NPComponent(
            ...
            triple_modulation=getattr(config, 'triple_modulation', False)  # Triple modulation (gate/up/down)
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(modulation_output[0], tuple):
            # Check if triple modulation (3 tuples) or dual modulation (2 tuples)
            if len(modulation_output) == 3:
                # Triple modulation: separate for gate, up, and down
                (v_a_gate, v_b_gate), (v_a_up, v_b_up), (v_a_down, v_b_down) = modulation_output
                mlp_output = self._apply_triple_modulated_mlp(
    ...
    def _apply_triple_modulated_mlp(
        self,
        hidden_states: torch.Tensor,
        v_a_gate: torch.Tensor,
        v_b_gate: torch.Tensor,
        v_a_up: torch.Tensor,
        v_b_up: torch.Tensor,
        v_a_down: torch.Tensor,
        v_b_down: torch.Tensor
    ) -> torch.Tensor:
        """
        Apply MLP with triple weight modulation (gate, up, and down projections).

        This extends dual modulation by also modulating the down projection,
        providing complete control over all MLP transformations.

        Math for down modulation:
        - W_down: (d_model × d_ffn)
        - v_a_down: (batch, seq, d_ffn) - matches input dimension of down
        - v_b_down: (batch, seq, d_model) - matches output dimension of down
        - Efficient: W_down @ intermediate + v_b_down * (v_a_down · intermediate)

        Args:
            hidden_states: Input to MLP (batch_size, seq_len, hidden_size)
            v_a_gate, v_b_gate: Modulation for gate projection
            v_a_up, v_b_up: Modulation for up projection
            v_a_down, v_b_down: Modulation for down projection
        ...
        # Apply gate and up modulations
        gate_modulated = gate_base + gate_modulation
        up_modulated = up_base + up_modulation

        # SwiGLU computation with modulated weights
        intermediate = F.silu(gate_modulated) * up_modulated  # (batch, seq, intermediate)

        # Apply down projection with modulation
        down_base = F.linear(intermediate, W_down)  # (batch, seq, hidden)

        # Compute down modulation
        # CRITICAL: v_a_down has d_ffn dimension, v_b_down has d_model dimension
        if v_a_down.dim() == 3:
            # Rank-1 modulation for down
            v_a_dot_intermediate = torch.sum(v_a_down * intermediate, dim=-1, keepdim=True)
            down_modulation = v_b_down * v_a_dot_intermediate
        else:
            # Rank-k modulation for down
            intermediate_expanded = intermediate.unsqueeze(2)  # (batch, seq, 1, intermediate)
            v_a_dot_intermediate = torch.sum(v_a_down * intermediate_expanded, dim=-1, keepdim=True)
            down_modulations = v_b_down * v_a_dot_intermediate
            down_modulation = torch.sum(down_modulations, dim=2)

        # Apply down modulation
        output = down_base + down_modulation
